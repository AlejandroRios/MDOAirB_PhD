...
def family_network_optimization_check(computation_mode, airports_keys, acft,index):
    ...
    return
def network_optimization(computation_mode, airports_keys, acft1):
    ...
    if (computation_mode == 0):
        ...
        aircrafts01 = LpVariable.dicts("aircrafts1",(arcs),0,30,LpInteger)
        ...
        for i in arcs:
            prob += flow[i] <= aircrafts01[i]*avg_capacity1
    ...
    for i in range(len(list_of_pax)):
        if (list_of_pax[i] <= 0):
            ...
            doc_1_list.append(docs_list1[i]*list_aircrafts1[i])
        # revenue_1_list.append(1.1*list_of_pax[i]*average_ticket_price01)
    ...
    for i in range(len(airports_keys)):
        for j in range(len(airports_keys)):
            list_of_airplanes_processed[i][j]= aircrafts1[i][j] 
    ...
    for i in range(len(airports_keys)):
        for k in range(len(airports_keys)):
            ...
            airplanes_ik[(airports_keys[i],airports_keys[k])] = aircrafts1[i][k] 
    ...
    DOC_tot = np.sum(DOC_proccessed1)
    ...
    return profit, vehicle01, kpi_df1_1, kpi_df2_1, airplanes_ik
